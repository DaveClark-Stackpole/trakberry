...
def hrly_display(request):   # This will return a tuple with hourly prod summary on last hour for each p_cell
	...
	try:
	  sql = "SELECT Id,employee,kiosk_id,supervisor,help_message FROM tkb_help WHERE closed = '%d'" %(help_closed)
	  cur.execute(sql)
	  tmp2 = cur.fetchall()
	  tmp3 = tmp2[0]
...
def email_hour_check2(request):
	...
	# This section is to email out any entries below 70% Target
	try:
		zql = "SELECT * FROM sc_production1 where low_production = '%d'" %(production_check)
		cursor.execute(zql)
		zmp = cursor.fetchall()
		tmp2 = zmp[0]
		email_manual1(zmp,reason2,request)
	...
	# This section is to email out any entries misses
	# try:
...
def email_manual1(tmp,reason,request):
	db, cursor = db_set(request)  
	b = "\r\n"
	ctr = 0
	...
	for x in tmp:
		...
		nnm = nm + '.0'
		try:
			zql = "SELECT * FROM tkb_manpower where Clock = '%s'" %(nnm)
			cursor.execute(zql)
			zmp = cursor.fetchall()
			zzmp = zmp[0]
			nm = zzmp[1]
